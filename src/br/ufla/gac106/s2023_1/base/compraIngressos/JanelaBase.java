...
/**
 * Classe que define uma Janela com um painel superior com intruções, um painel
 * central a ser defindo
 * por uma subclasse e um painel inferior com, possivelmente, botões Voltar,
 * Avançar e Finalizar
 * 
 * Obs.: repare que, diferentemente do exemplo usando em sala de aula, a classe
 * JanelaBase herda de
...
public abstract class JanelaBase extends JFrame {
     ...
     * @param janelaAnterior    Janela a ser chamada ao clicar no botão Voltar (se
     *                          ele não estiver visível passe null)
    ...
    public JanelaBase(String titulo, String instrucoes, int largura, int altura,
        ...
        FlatIntelliJLaf.setup(); // define layout com tema escuro
        ...
        // cria o painel inferior com, possivelmente, os botões Voltar, Avançar e
        // Finalizar
    ...
    /**
     * Cria o painel inferior da janela com, possivelmente, os botões Voltar,
     * Avançar e Finalizar
     */
    ...
    private void tratarBotaoVoltar(boolean temBotaoVoltar, JPanel painelInferior) {
        if (temBotaoVoltar) {
            ...
            // Ao clicar em Voltar é chamado o método `aoVoltar`. Se ele retornar true fecha
            // a janela e exibe a janela anterior.
    ...
    private void tratarBotaoAvancar(boolean temBotaoAvancar, JPanel painelInferior) {
        if (temBotaoAvancar) {
            ...
            // Ao clicar em Avançar é chamado o método `aoAvancar`. Se ele retornar true a
            // tela é fechada.
    ...
    private void tratarBotaoFinalizar(boolean temBotaoFinalizar, JPanel painelInferior) {
        if (temBotaoFinalizar) {
            ...
            // Ao clicar em Finalizar é chamado o método `aoFinalizar`. Se ele retornar true
            // a tela é fechada.
    ...
    /**
     * Método a ser sobrescrito para tratar o que acontece quando o usuário clica em
     * "Voltar"
     * Obs.: não é necessário tratar a exibição da janela anterior (isso já é
     * tratado na classe JanelaBase)
     * 
     ...
     * "Finalizar"
     * 
     * @return O retorno do método é usado para indicar se vai finalizar mesmo ou
     *         não
     */
